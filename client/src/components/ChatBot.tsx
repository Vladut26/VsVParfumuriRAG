...
const sendChat = async (message: string, history: Message[]): Promise<{ reply: string; products: RecProduct[] }> => {
  const res = await APIService.chat({ message, history: history.map(m => ({ role: m.role, content: m.content })) });
  const data = res.data as { reply: string; products?: RecProduct[] };
  return { reply: data.reply || "", products: data.products || [] };
};

const INITIAL_SUGGESTIONS = [
  "Parfum floral pentru ea",
  "Cadou de lux sub 500 RON",
  "Parfum cu oud si amber",
  "Ce e popular acum?",
];
...
function getFollowUpSuggestions(lastReply: string): string[] {
  ...
  if (lower.includes("ron") || lower.includes("pret")) suggestions.push("Ai ceva mai ieftin?");
  if (lower.includes("floral") || lower.includes("trandafir")) suggestions.push("Si ceva oriental?");
  if (lower.includes("cadou") || lower.includes("ea")) suggestions.push("Si un parfum pentru el?");
  if (lower.includes("stoc") || lower.includes("epuizat")) suggestions.push("Ce alternative ai?");
  const generic = ["Arata-mi mai multe optiuni", "Care e cel mai vandut?", "Vreau ceva sub 300 RON", "Compara ultimele 2"];
  while (suggestions.length < 3 && generic.length > 0) {
    const pick = generic.splice(Math.floor(Math.random() * generic.length), 1)[0];
...
const ChatBot: FC = () => {
  ...
  useEffect(() => { endRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, loading, followUps]);
  ...
  // ── MUTAT AICI (REZOLVĂ HOISTING & SCOPE ERRORS) ───────────────────
  const send = useCallback(async (text?: string) => {
    ...
    try {
      ...
      const { reply, products } = await sendChat(msg, history);
      setMessages(prev => [...prev, { role: "assistant", content: reply, products }]);
      setFollowUps(getFollowUpSuggestions(reply));
    } catch {
      setMessages(prev => [...prev, { role: "assistant", content: "Imi pare rau, am intampinat o eroare. Te rog sa incerci din nou." }]);
      setFollowUps([]);
    } finally { setLoading(false); }
  }, [input, loading, messages]);

  // ── URMEAZĂ EVENT LISTENER-UL CARE DEPINDE DE STRATEGIA DE SEND ───
  useEffect(() => {
    const handler = (e: Event) => {
      const msg = (e as CustomEvent).detail as string;
      if (msg) { setOpen(true); setTimeout(() => void send(msg), 200); }
    };
    window.addEventListener("vsv:open-chat", handler);
    return () => window.removeEventListener("vsv:open-chat", handler);
  }, [send]); // Adăugat 'send' ca dependență cerută de regulă
  ...
  const clearChat = () => { setMessages([GREETING]); setFollowUps([]); };

  // ── Product card component ──────────────────────────────────────
  const ProductCard: FC<{ product: RecProduct }> = ({ product: p }) => (
    <div className="flex gap-2.5 bg-white/5 border border-white/8 rounded-xl p-2.5 hover:bg-white/8 transition-colors">
    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full flex items-center justify-center
          ...
          ${open
            ? "bg-[var(--noir)] text-white scale-95"
      ...
      {open && (
        <div
          className="fixed bottom-24 right-4 z-50 w-[calc(100vw-2rem)] sm:w-[420px]
                     rounded-2xl shadow-2xl overflow-hidden flex flex-col
                     animate-chat-slide-up border border-[var(--noir-lighter)]"
          ...
          <div className="px-5 py-4 flex items-center gap-3 flex-shrink-0 border-b border-white/5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-[var(--gold)]/60 mt-0.5 tracking-wider uppercase">
                {loading ? "Analizeaza..." : "Fragrance Concierge"}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${loading ? "bg-[var(--gold)] animate-pulse" : "bg-emerald-400"}`} />
              <button onClick={clearChat} className="text-[10px] text-white/30 hover:text-white/60 uppercase tracking-wider transition-colors">Reset</button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 min-h-0">
            ...
            {messages.length === 1 && !loading && (
              <div className="flex flex-wrap gap-2 mt-1">
                {INITIAL_SUGGESTIONS.map(s => (
                  <button key={s} onClick={() => void send(s)} disabled={loading}
                    className="px-3 py-1.5 rounded-full text-[11px] font-medium border border-[var(--gold)]/20
                               text-[var(--gold-light)]/70 hover:bg-[var(--gold)]/10 hover:border-[var(--gold)]/40
            ...
            {messages.map((m, i) => (
              <div key={i}>
                ...
                {/* Product hero cards — shown after assistant messages */}
                {m.role === "assistant" && m.products && m.products.length > 0 && (
                  <div className="pl-9 mt-2 space-y-2 animate-chat-bubble">
                    ...
                    {m.products.length > 4 && (
                      <button
                        onClick={() => navigate("/")}
                        className="w-full text-center py-2 text-[11px] text-[var(--gold)]/50 hover:text-[var(--gold)]
